...
% samplingFreq/bytes*ms_of_data
settings.skipNumberOfBytes      = 0; %200e6;
settings.skipNumberOfSamples    = 0;
%% Raw signal file name and other parameter ===============================
% This is a "default" name of the data file (signal record) to be used in
% the post-processing mode
% settings.fileName           = 'BDS_B2IdataSchar2.bin';
...
% up acquisition; Full constellation of Beidou has 37 satellites.
settings.acqSatelliteList   = [6:58];  % [PRN numbers]  
% Band around IF to search for satellite signal. Depends on max Doppler
settings.acqSearchBand      = 10;           %[kHz]
% Threshold for the signal presence decision rule
settings.acqThreshold       = 2; %1.5 BDS B2I / 1.75 BDS B1I;
% Sampling rate threshold for downsampling 
settings.resamplingThreshold    = 9e6;            % [Hz]
% Enable/dissable use of downsampling for acquisition
settings.resamplingflag         = 0;              % 0 - Off
                                                  % 1 - On
% Enable/dissable use of downsampling for acquisition
settings.stepSize = 125;            % [] - Found from samplingFreq and codeLength
...
settings.pllNoiseBandwidth       = 35;      %[Hz]
...
% If there are MEO satellites, then settings.startOffset should be 68.802.
% But current data has no MEO. So it would be 120 ms for GEO & IGSO. Their
% alitude is 35,786 km from MSL.
settings.startOffset        = 120;         %[ms] Initial sign. travel time
