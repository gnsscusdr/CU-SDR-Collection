...
%--------------------------------------------------------------------------
% Modified for Beidou by Daehee Won at Sep. 26, 2013
% Updated by Yafeng Li at April 18th, 2017
...
SOW  = inf(1, settings.numberOfChannels);
...
for channelNr = activeChnList
    ...
    [eph(PRN),subFrameStart(channelNr),SOW(channelNr)] = ...
        NAVdecoding(trackResults(channelNr).I_P,PRN,settings);  %#ok<AGROW>
...
% time, and Sat health flag. Any SV with the health falg unavailable is 
% excluded from the processing list   
SatH1Index = zeros(1, length(activeChnList));
for ind = 1: length(activeChnList)
    if eph(trackResults(activeChnList(ind)).PRN).SatH1 == 0
        SatH1Index(ind) =  activeChnList(ind);
    end

end

% Sv list used to do fix 
readyChnList = intersect(SatH1Index,activeChnList);
...
%% Find pseudoranges ======================================================
    ...
    [navSolutions.rawP(:, currMeasNr),transmitTime,localTime]=  ...
                     calculatePseudoranges(trackResults,subFrameStart,SOW, ...
