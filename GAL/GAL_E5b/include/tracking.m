...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        for loopCnt =  1:settings.msToProcess
            ...
            tcode2      = ceil(tcode) + 1;
            earlyCode   = E5bICode(tcode2);
            % For pilot channel signal tracking
            if (settings.pilotTRKflag == 1)
                earlyCodeE1c   = E5bQcode(tcode2);
            end
            ...
            tcode       = (remCodePhase+earlyLateSpc) : ...
                ...
                ((blksize-1)*codePhaseStep+remCodePhase+earlyLateSpc);
            tcode2      = ceil(tcode) + 1;
            lateCode    = E5bICode(tcode2);
            % For pilot channel signal tracking
            if (settings.pilotTRKflag == 1)
                lateCodeE1c   = E5bQcode(tcode2);
            end
            ...
            tcode2      = ceil(tcode) + 1;
            promptCode  = E5bICode(tcode2);
            % For pilot channel signal tracking
            if (settings.pilotTRKflag == 1)
                promptCodeE1c   = E5bQcode(tcode2);
            end
            ...
            if (settings.pilotTRKflag == 1)
                ...
                % carrier phase tracking error for pilot signal (E5bQ)
                carrErrorE1c = atan(imag(QI)/real(QI))/ (2.0 * pi);
                
                % As the E5bI and E5bQ power is the same, so a simple avergae
