...
%       channel         - frequency channel, carrier frequencies and code 
%                       phases of all satellites to be tracked (prepared 
%                       by preRum.m from acquisition results).
...
caCode = generateCAcode(0,settings.codeFreqBasis,511);
% Then make it possible to do early and late versions
caCode = [caCode(511) caCode caCode(1)];

%% Initialize tracking variables ==========================================

codePeriods = settings.msToProcess;    % For GLONASS one C/A code is one ms
...
for channelNr = 1:settings.numberOfChannels
    ...
    % Only process if the channel's acquisition was successful
    if (channel(channelNr).status ~= '-')
        % Save additional information - each channel's tracked frequency
        % channel
        trackResults(channelNr).PRN     = channel(channelNr).K;
        ...
        for loopCnt =  1:codePeriods
            ...
            if (dataAdaptCoeff==2)
                ...
                rawSignal = rawSignal2 + 1i .* rawSignal1;  %transpose vector
            end
