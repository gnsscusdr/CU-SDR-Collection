...
%--------------------------------------------------------------------------
%                         CU Multi-GNSS SDR
% (C) Developed for GLONASS L1/L2 SDR by Yafeng Li, Nagaraj C. Shivaramaiah
% and Dennis M. Akos.
...
caCodes2ms = [caCode zeros(1,samplesPerCode)];
% Perform DFT of the C/A code
...
% Prepare C/A code with 40 ms
caCode40ms = generateCAcode(0,settings.samplingFreq,samplesPerCode*40);
% Phase points of the local carrier wave for 40ms carrier
...
for K = settings.acqSatelliteList
    ...
    for freqBinIndex = 1:numberOfFreqBins
        ...
        coarseFreqBin(freqBinIndex) = settings.IF - settings.freqSpacing * K + ...
    ...
    acqResults.peakMetric(K+8) = peakSize/sigPower/settings.acqNonCohTime;
    ...
    if acqResults.peakMetric(K+8) > settings.acqThreshold
        ...
        for fineBinIndex = 1 : numOfFineBins
            ...
            for comIndex = 1:20
                ...
                comPower = abs(sum(sumPerCode(comIndex:comIndex+9)) - ...
                    sum(sumPerCode(comIndex+10:comIndex+19)));
        ...
        acqResults.carrFreq(K+8)  = fineFreqBins(maxFinBin);
        % Save code phase acquisition result
        acqResults.codePhase(K+8) = codePhase;
        ...
        if (exist('oldFreq', 'var') && settings.resamplingflag == 1)
            % Find code phase
            acqResults.codePhase(K+8) = floor((codePhase - 1)/ ...
            ...
            if (settings.IF >= settings.samplingFreq/2)
                ...
                doppler = IF_temp - acqResults.carrFreq(K+8);
            else
                doppler = acqResults.carrFreq(K+8) - settings.IF;
            end
            
            % Carrier freq. corresponding to orignal sampling freq
            acqResults.carrFreqcarrFreq(K+8) = doppler + oldIF;
        end
    else
