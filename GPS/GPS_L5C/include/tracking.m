...
for channelNr = 1:settings.numberOfChannels
    ...
    if (channel(channelNr).PRN ~= 0)
        ...
        L5ICode = generateL5Icode(channel(channelNr).PRN,settings);
        % Then make it possible to do early and late versions
        L5ICode = [L5ICode(settings.codeLength) L5ICode L5ICode(1)]; 
        ...
        for loopCnt =  1:codePeriods
           ...
           %% Generate the carrier frequency to mix the signal to baseband -----------
            ...
            % chaannel signals
            if (settings.pilotTRKflag == 1)                
                % Pilot channel(L5Q) carrier phase is pi/2 rad ahead of the
                % data channel (L5I) carrier phase. Here we rotate the L5Q
                % phase pi/2 back to the L5I phase. 
            ...
            % chaannel signals
            if (settings.pilotTRKflag == 1)
                codeErrorQ = (sqrt(I_EQ^2 + Q_EQ^2) - sqrt(I_LQ^2 + Q_LQ^2)) / ...
            ...
            if (settings.pilotTRKflag == 1)
                trackResults(channelNr).Pilot_I_P(loopCnt) = I_PQ ;
                trackResults(channelNr).Pilot_Q_P(loopCnt) = Q_PQ;
            end
            %% CNo calculation --------------------------------------------------------
