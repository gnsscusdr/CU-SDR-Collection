function [acqResults, trkResults,navResults]  = postProcessing(settings, acqPath, trkPath, navPath)
...
% 5) Plot the results.
    ...
    %% Acquisition ============================================================
        ...
        if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
            ...
            % Find number of samples per L2 CM spreading code(20 ms)
            samplesPerCode = round(settings.samplingFreq / ...
                               (settings.codeFreqBasis / settings.codeLength));
            
           
            % Read data for acquisition (3 CM codes should be enough)
            data  = fread(fid, dataAdaptCoeff*60*samplesPerCode/20, settings.dataType)';
