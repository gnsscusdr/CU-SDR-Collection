...
bar(hAxes, -7:13, acqResults.peakMetric);

title (hAxes, 'Acquisition results');
xlabel(hAxes, 'Freq. Channel (no bar - SV is not in the acquisition list)');
ylabel(hAxes, 'Acquisition Metric');

oldAxis = axis(hAxes);
axis  (hAxes, [-8, 7, 0, oldAxis(4)]);
...
hold(hAxes, 'on');
bar (hAxes, -7:13, acquiredSignals, 'FaceColor', [0 0.8 0]);
hold(hAxes, 'off');
