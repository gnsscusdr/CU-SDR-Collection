...
channel.K               = 0;    % Frequency channel of the tracked satellite
...
[junk, Kindexes]          = sort(acqResults.peakMetric, 2, 'descend');
...
for ii = 1:min([settings.numberOfChannels, sum(acqResults.carrFreq ~= 0)])
    channel(ii).K            = Kindexes(ii)-8;
    channel(ii).acquiredFreq = acqResults.carrFreq(Kindexes(ii));
    channel(ii).codePhase    = acqResults.codePhase(Kindexes(ii));
